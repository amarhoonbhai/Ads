        ...
        try:
            ...
            for message in reversed(messages):
                if message.message is None and not message.media:
                    continue  # Skip MessageService or empty messages
                for group in groups:
                    try:
...
def main():
    ...
    async def handler(event):
        if event.sender_id == OWNER_ID and event.raw_text.startswith('.time'):
            match = re.match(r'\.time\s+(\d+)([mh]?)', event.raw_text)
            if match:
                ...
                print(f"[Owner Command] Updated CYCLE_DELAY_MIN to {CYCLE_DELAY_MIN} minutes from chat")
                await event.respond(f"✅ Time is successfully changed to {readable}")
